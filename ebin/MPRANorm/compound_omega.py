...
def marginal_loglik_charlier(processed_data: list[list[tuple[jnp.ndarray, jnp.ndarray]]], r: jnp.ndarray, p: jnp.ndarray,
                             theta: jnp.ndarray, lambda_poi: jnp.ndarray, moments=None, lambda_moment: int = 0,
    ...
    taus, tau_logweights = jax.vmap(compute_nodes_and_logweights,
                                    in_axes=(None, 0, None))(c_nodes, phi_hat[j:],
                                                             Rules.Charlier)
    ...
    right_poi = scp_poisson.isf(eps, MAX_POI)
    taus_poi = jnp.arange(0, right_poi + 1)
    tau_logweights_poi = jax.scipy.stats.poisson.logpmf(taus_poi, lambda_poi_val)

    tau_logweights = tau_logweights 
    loglik = jnp.zeros((B, K, N), dtype=prec_float)
    loglik_poi = jnp.zeros((B, K, N), dtype=prec_float)
    ...
    for b in range(B):
        ...
        marg_nb_poi_b = jnp.zeros((K, N), dtype=prec_float)
        for m in range(M):
            x, inv_ind = processed_data[b][m]
            x = x.astype(prec_int)
            marg_nb_s = logpmf(x, r[b, m], p[b, m], taus, tau_logweights)
            marg_nb_s = marg_nb_s + phi_hat[None, None]
            marg_nb = marg_nb + marg_nb_s[:,inv_ind]
            
            marg_nb_s_poi = logpmf_poi(x, r[b, m], p[b, m], taus_poi, tau_logweights_poi)
            marg_nb_poi_b = marg_nb_poi_b + marg_nb_s_poi[:, inv_ind]
            
        marg_nb = marg_nb + logweights[None, None]
        loglik = loglik.at[b].set(logsumexp(marg_nb, axis=-1)) 
        loglik_poi = loglik_poi.at[b].set(marg_nb_poi_b)
        
    return jnp.transpose(loglik - const, (0, 2, 1)), jnp.transpose(loglik_poi + lambda_moment * jnp.log(lambda_poi_val), (0, 2, 1))
...
def marginal_loglik(processed_data: list[list[tuple[jnp.ndarray, jnp.ndarray]]], r: jnp.ndarray, p: jnp.ndarray,
    ...
    # Append the regular poisson deterministic parameters to the evaluation batch
    log_phi_hat_all = jnp.append(log_phi_hat, jnp.log(lambda_poi_val))
    ...
    loglik_poi = jnp.zeros((B, N, K), dtype=prec_float)
    if make_valid_mixture:
        const = logsumexp(logweights + phi_hat * M)
    else:
        const = jnp.log(M * beta + 1) * alpha + gammaln(alpha)
    const = const + jnp.log(M + 1 / beta) * lambda_moment
    if not high_precision:
        const = const.astype(jnp.float32)
        logweights = logweights.astype(jnp.float32)
    ...
    for b in range(B):
        marg_nb = jnp.zeros((N, K, num_nodes + 1), dtype=prec_float)
        for m in range(M):
            x, inv_ind = processed_data[b][m]
            x = x.astype(prec_int)
        ...
        marg_nb_gamma = marg_nb[..., :-1] + logweights[None, None]
        marg_nb_poi_b = marg_nb[..., -1] - M * lambda_poi_val
        
        loglik = loglik.at[b].set(logsumexp(marg_nb_gamma, axis=-1))
        loglik_poi = loglik_poi.at[b].set(marg_nb_poi_b)
        
    return loglik - const, loglik_poi + lambda_moment * jnp.log(lambda_poi_val)
...
def update_weights(T: jnp.ndarray, C: jnp.ndarray, T_Y: jnp.ndarray = None, eps=1e-12):
    ...
    if T_Y is not None:
        new_omega = jnp.mean(T_Y)
        new_omega = jnp.clip(new_omega, eps, 1.0 - eps)
        return new_weights, new_pi, jnp.array([new_omega])

    return new_weights, new_pi

@partial(jax.jit, static_argnames=( 'num_nodes', 'valid_mixture'))
def compute_responsibilities(processed_data: _prepared_data, 
                             r: jnp.ndarray, p: jnp.ndarray, theta: jnp.ndarray, lambda_poi: jnp.ndarray,
                             pi: jnp.ndarray, w: jnp.ndarray, omega: jnp.ndarray, num_nodes: int,
                             valid_mixture: bool = False,
                             eps=1e-12) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    logP_gamma, logP_poi = marginal_loglik(processed_data, make_valid_mixture=valid_mixture,
                           num_nodes=num_nodes, r=r, p=p, theta=theta, lambda_poi=lambda_poi,
                           high_precision=True)
    
    M = len(processed_data[0])
    B, N, K = logP_gamma.shape
    ...
    log_omega = jnp.log(jnp.clip(omega[0], eps, 1.0 - eps))
    log_one_minus_omega = jnp.log1p(-jnp.clip(omega[0], eps, 1.0 - eps))
    ...
    # Eval. Gamma
    log_term2_gamma = log_one_minus_pi_b + logP_gamma
    log_term_per_indiv_and_comp_gamma = jnp.logaddexp(log_term1, log_term2_gamma)
    log_P_Xn_given_Znk_gamma = jnp.sum(log_term_per_indiv_and_comp_gamma, axis=0)
    
    # Eval. Poisson
    log_term2_poi = log_one_minus_pi_b + logP_poi
    log_term_per_indiv_and_comp_poi = jnp.logaddexp(log_term1, log_term2_poi)
    log_P_Xn_given_Znk_poi = jnp.sum(log_term_per_indiv_and_comp_poi, axis=0)

    log_P_Xn_given_Znk = jnp.logaddexp(
        log_one_minus_omega + log_P_Xn_given_Znk_gamma,
        log_omega + log_P_Xn_given_Znk_poi
    )
    ...
    log_R_gamma_term = (log_one_minus_omega + log_one_minus_pi_b + logP_gamma 
                        + log_P_Xn_given_Znk_gamma[None, :, :] - log_term_per_indiv_and_comp_gamma)
    log_R_poi_term = (log_omega + log_one_minus_pi_b + logP_poi 
                      + log_P_Xn_given_Znk_poi[None, :, :] - log_term_per_indiv_and_comp_poi)
    
    log_R_gamma = log_R_gamma_term + log_weights[None, None, :] - log_P_Xn[None, :, None]
    log_R_poi = log_R_poi_term + log_weights[None, None, :] - log_P_Xn[None, :, None]

    R_gamma = jnp.exp(log_R_gamma)
    R_poi = jnp.exp(log_R_poi)

    C = 1.0 - jnp.sum(R_gamma + R_poi, axis=2)
    
    log_P_Yn1_Xn = log_omega + logsumexp(log_weights + log_P_Xn_given_Znk_poi, axis=1)
    T_Y = jnp.exp(log_P_Yn1_Xn - log_P_Xn)

    return T, C, (R_gamma, R_poi), T_Y, log_P_Xn.sum() / (B * M * N)
...
def get_starting_values(data: jnp.ndarray, K: int, theta, r, p, w, pi, omega=None, lambda_poi=None):
    ...
    if omega is None:
        omega = jnp.array([1e-2])
    if lambda_poi is None:
        lambda_poi = jnp.array([1.0])
...
@partial(jax.jit, static_argnames=('num_nodes', 'high_precision',
                                   'valid_mixture'))
def Q_dist(r: jnp.ndarray, p: jnp.ndarray, theta: jnp.ndarray, lambda_poi: jnp.ndarray, R: tuple,
           processed_data: _prepared_data, num_nodes: int,
           valid_mixture: bool = False, high_precision: bool = True):
    logP_gamma, logP_poi = marginal_loglik(processed_data, 
                           r=r, p=p, theta=theta, lambda_poi=lambda_poi, num_nodes=num_nodes,
                           make_valid_mixture=valid_mixture,
                           high_precision=high_precision)
    R_gamma, R_poi = R
    
    return -(logP_gamma * R_gamma + logP_poi * R_poi).mean()
    
@partial(jax.jit, static_argnames=('num_nodes', 'high_precision',
                                   'valid_mixture'))
def negloglik(r: jnp.ndarray, p: jnp.ndarray, theta: jnp.ndarray, lambda_poi: jnp.ndarray, w: jnp.ndarray, 
              pi: jnp.ndarray, omega: jnp.ndarray,
    ...
    for n in range(num_iters):
        try:
            ...
            for funr, slicer, bounds, names, (b, m_slc) in tqdm(rp_funs):
                ...
                x0 = jnp.asarray(x0)
                fixed_params = {'theta': params['theta'], 'lambda_poi': params['lambda_poi']}
                if 'w' in params:
                ...
                if R is None:
                    ...
                    if R is not None:
                        fun = partial(funr, R=(R[0][b:b+1], R[1][b:b+1]), **fixed_params)
                    else:
                        fun = partial(funr, **fixed_params)
                with warnings.catch_warnings(action="ignore"):
...
def fit_em(data: jnp.ndarray, num_mixtures: int, 
    ...
    params = {'r': r, 'p': p, 'theta': theta, 'lambda_poi': lambda_poi}
    ...
    for n_iter in range(num_iters):
        ...
        try:
            ...
            t0 = time()
            print('E-step...')
            full_x = {**params, 'w': w, 'pi': pi, 'omega': omega}
            flat_x, unravel = ravel_pytree(full_x)
            T, C, R, T_Y, loglik = calc_responsibilities(pi=pi, w=w, omega=omega, **params)
            if n_iter > 0:
                print(loglik, prev_loglik)
            w, pi, omega = update_weights(T, C, T_Y)
            w = w[:-1]
            print(f"theta = {params['theta']}, lambda_poi = {params['lambda_poi']}, omega = {omega}, w = {w}")
            ...
            full_g = {**params, 'w': w, 'pi': pi, 'omega': omega}
            ...
            if used_aa:
                T, C, R, T_Y, extra_loglik = calc_responsibilities(pi=full_extra['pi'], w=full_extra['w'],
                                                              omega=full_extra['omega'],
                                                              **{k: v for k, v in full_extra.items() if k not in ['w', 'pi', 'omega']})
    ...
    T, C, R, T_Y, loglik = calc_responsibilities(**params, pi=pi, w=w, omega=omega)
    w, pi, omega = update_weights(T, C, T_Y)
    params['w'] = w
    params['pi'] = pi
    params['omega'] = omega
...
def fit_mle(data: jnp.ndarray, num_mixtures: int, 
    ...
    params = {'r': r, 'p': p, 'theta': theta, 'lambda_poi': lambda_poi, 'w': w, 'pi': pi, 'omega': omega}
    ...
    if cd:
        ...
        params, t = optimize(fun=fun, data=data, processed_data=processed_data,
                             ftol=ftol,
                                        ...
                                        optimizer='SLSQP',)
    T, C, R, T_Y, loglik = compute_responsibilities(valid_mixture=True,  processed_data=processed_data,
                                               num_nodes=num_nodes,
                                               **params)
    w = params['w']
...
def calc_tau_means(data, params, responsibilities, 
    ...
    theta, r, p, lambda_poi = params['theta'], params['r'], params['p'], params['lambda_poi']
    B, M, N = data.shape
    ...
    logmoments_gamma, logmoments_poi = logmoments
    
    moments_gamma = jnp.exp(logmoments_gamma[1:] - logmoments_gamma[:1])
    moments_poi = jnp.exp(logmoments_poi[1:] - logmoments_poi[:1])
    
    R_gamma, R_poi = responsibilities
    tau_means = (R_gamma * moments_gamma + R_poi * moments_poi).sum(axis=-1).transpose(1, 0, 2)
    return tau_means

def estimate_lambda(data, params, responsibilities, batch_size: int = 32,
                    num_nodes: int = 100) -> tuple[np.ndarray, np.ndarray]:
    B, M, N = data.shape
    theta, r, p, lambda_poi = params['theta'], params['r'], params['p'], params['lambda_poi']
    processed = prepare_data(data)
    ...
    logmarginal_gamma, logmarginal_poi = momfun(0)
    
    log1_gamma, log1_poi = momfun(1)
    first_moment_gamma = jnp.exp(log1_gamma - logmarginal_gamma)
    first_moment_poi = jnp.exp(log1_poi - logmarginal_poi)
    
    log2_gamma, log2_poi = momfun(2)
    second_moment_gamma = jnp.exp(log2_gamma - logmarginal_gamma)
    second_moment_poi = jnp.exp(log2_poi - logmarginal_poi)
    
    R_gamma, R_poi = responsibilities
    mean = (first_moment_gamma * R_gamma + first_moment_poi * R_poi).sum(axis=-1)
    var = (R_gamma * (second_moment_gamma - first_moment_gamma ** 2) + R_poi * (second_moment_poi - first_moment_poi ** 2)).sum(axis=-1)
    return mean, var

@partial(jax.jit, static_argnames=('sample_ind', 'normalized'))
def _estimate_log_posterior_jit(tau, sample_ind, X, r, p, w, pi, omega, lambda_poi,
    ...
    logweights_gamma = logweights + jnp.log1p(-omega[0])
    logweights_poi = -M * lambda_poi_val + jnp.log(omega[0])
    logweights_all = jnp.append(logweights_gamma, logweights_poi)
...
def estimate_log_posterior(tau: jnp.ndarray, sample_ind: int, result: LatentCountsResult, X: np.ndarray, normalized: bool = False) -> np.ndarray:
    ...
    lambda_poi = jnp.asarray(params.get('lambda_poi', [1.0]))
    ...
    omega = jnp.asarray(params.get('omega', [1e-2]))
    ...
    log_posterior = _estimate_log_posterior_jit(
        ...
        X=X_jnp,
        r=r, p=p, w=w, pi=pi, omega=omega, lambda_poi=lambda_poi,
...
def infer_latent_counts(data: np.ndarray, num_mixture_components: int,
    ...
    if 'omega' not in params:
        params['omega'] = jnp.array([1e-2])
    if 'lambda_poi' not in params:
        params['lambda_poi'] = jnp.array([1.0])
    ...
    taus = calc_tau_means(data, params, R, num_nodes=num_nodes)
    taus = np.asarray(taus)
    lambdas, lambdas_var = estimate_lambda(data, params, R)
    
    R_gamma, R_poi = R
    R_total = np.asarray(R_gamma + R_poi)
